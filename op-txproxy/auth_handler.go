...
// authHandler detects when authentication information is present on the request.
// If so, it will validate and set authenicated caller in the request context. It does not
// reject if authentication information is missing. It is up to the request handler to do so.
...
func (h *authHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// The request body can only be read once. Adjust here so it's inspectable
	var buf bytes.Buffer
	body := io.TeeReader(r.Body, &buf)
	r.Body = io.NopCloser(&buf)

	// Since this middleware runs prior to the server, we need to manually apply the body limit when reading.
	bodyBytes, err := io.ReadAll(io.LimitReader(body, int64(defaultBodyLimit)))
	if err != nil {
	...
	h.next.ServeHTTP(w, r.Clone(newCtx))
}
